...
def _era5_check(retrieval: omegaconf.DictConfig):
    ...
    # make directory for outputs if needed
    os.makedirs(retrieval.output_dir, exist_ok=True)

    ds = xr.open_dataarray(retrieval.file_path)
    logger.info(f'Checking file: {retrieval.file_path}. Output plots to {retrieval.output_dir}')
    ...
    plt.xlabel('Time')
    plt.savefig(os.path.join(retrieval.output_dir, 'global_average_timeseries.png'))
    plt.close()
    ...
    if 'plot_times' in retrieval.keys():
        ...
        for plot_time in pbar:
            ...
            plt.savefig(os.path.join(retrieval.output_dir, f'spatial_map_{plot_time.strftime("%Y%m%dT%H%M")}.png'))
            plt.close()
    return 
